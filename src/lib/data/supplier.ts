...
const supplierQueryKeys = {
  ...
  performance: (id: string, period: string) => [...supplierQueryKeys.all, 'performance', id, period] as const,
...
export function useSuppliers(filters?: SupplierFilters) {
  ...
  const createSupplierMutation = createMutation({
    ...
    onSuccess: (newSupplier) => {
      ...
      queryClient.setQueryData<PaginatedSuppliers>(
        supplierQueryKeys.list(filters),
        (oldData) => {
          if (!oldData) return { 
  ...
  const updateSupplierMutation = createMutation({
    mutationFn: ({ supplierId, supplierData }: { supplierId: string; supplierData: Partial<SupplierInput> }) => 
    ...
    onSuccess: (updatedSupplier) => {
      ...
      queryClient.setQueryData(
        supplierQueryKeys.detail(updatedSupplier.id),
        updatedSupplier
      );
  ...
  const deleteSupplierMutation = createMutation({
    ...
    onSuccess: (_, supplierId) => {
      ...
      queryClient.setQueryData<Supplier>(
        supplierQueryKeys.detail(supplierId),
        (oldData) => oldData ? { ...oldData, is_active: false } : oldData
      );
...
export function useSupplierProducts(supplierId: string) {
  ...
  return {
    ...
    isError,
    error,
    refetch: () => queryClient.invalidateQueries({ queryKey: supplierQueryKeys.products(supplierId) })
  };
}
  ...
  return {
    ...
    updateSupplierOptimistic: (supplierId: string, updates: Partial<Supplier>) => {
      ...
      queryClient.setQueriesData<Supplier>(
        { queryKey: supplierQueryKeys.details() },
        (oldData) => 
