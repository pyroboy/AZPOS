...
export function useModifiers() {
  ...
  const validateSelectionMutation = createMutation({
    mutationFn: (validationData: ValidateModifierSelection) => onValidateModifierSelection(validationData)
  });
  ...
  function setSorting(sort_by: ModifierFilters['sort_by'], sort_order: ModifierFilters['sort_order']) {
    updateFilters({ sort_by, sort_order });
  }

  // Validation helpers
  function validateSelection(modifierId: string, selectedOptions?: string[], textValue?: string, numberValue?: number) {
