...
export function useSalesReports() {
  ...
  const currentViewTotals = $derived(() => {
    const total_revenue = salesItems.reduce((sum: number, sale: SalesItem) => sum + sale.total_amount, 0);
    const total_quantity = salesItems.reduce((sum: number, sale: SalesItem) => sum + sale.quantity, 0);
  ...
  function setSorting(sort_by: SalesReportFilters['sort_by'], sort_order: SalesReportFilters['sort_order']) {
  ...
  function useSalesSummary(period: 'today' | 'week' | 'month' | 'year' | 'custom' = 'today', customFrom?: string, customTo?: string) {
  ...
  function exportSalesData(format: 'csv' | 'json' = 'csv') {
    if (format === 'csv') {
      const headers = ['Date', 'Product Name', 'SKU', 'Quantity', 'Unit Price', 'Total Amount', 'Reason'];
      const csvContent = [
        headers.join(','),
        ...salesItems.map(sale => [
