...
export function useCart() {
  ...
  const cartState = $derived(cartQuery.data ?? {
    items: [],
    discount: null,
  ...
  const cartTotals = $derived(cartTotalsQuery.data ?? {
    subtotal: 0,
    discount_amount: 0,
    tax: 0,
    total: 0,
    ...
    queryClient.setQueryData([...cartQueryKey, sessionId], (oldData: CartState | undefined) => {
      ...
      if (existingItemIndex >= 0) {
        ...
        updatedItems[existingItemIndex] = {
          ...
          subtotal: updatedItems[existingItemIndex].final_price * (updatedItems[existingItemIndex].quantity + quantity)
        };
