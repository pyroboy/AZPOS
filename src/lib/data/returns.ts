  ...
  const createReturnMutation = createMutation({
    ...
    onSuccess: (newReturn) => {
      ...
      queryClient.setQueryData<EnhancedReturnRecord[]>(
        ...
        (oldData) => oldData ? [newReturn, ...oldData] : [newReturn]
      );
    },
    onError: (error) => {
  ...
  const updateStatusMutation = createMutation({
    ...
    onSuccess: (updatedReturn) => {
      ...
      queryClient.setQueryData(
        returnsQueryKeys.detail(updatedReturn.id),
        updatedReturn
      );
...
export function useOptimisticReturnUpdate() {
  ...
  return {
    ...
    updateReturnOptimistic: (returnId: string, newStatus: EnhancedReturnRecord['status'], adminNotes?: string) => {
