...
export const userThemePreferencesSchema = z.object({
  ...
  switch_time_light: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).default('06:00'), // HH:MM format
  switch_time_dark: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).default('18:00'),
  custom_overrides: z.object({
    colors: colorPaletteSchema.partial().optional(),
    typography: typographySchema.partial().optional(),
    spacing: spacingSchema.partial().optional(),
    border_radius: borderRadiusSchema.partial().optional(),
...
export const themeStatsSchema = z.object({
  ...
  active_users_by_theme: z.record(z.string(), z.object({
    theme_id: z.string(),
    theme_name: z.string(),
    user_count: z.number(),
    percentage: z.number()
  ...
  most_customized_elements: z.array(z.object({
    element: z.string(),
