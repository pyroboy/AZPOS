...
export const transactionFiltersSchema = z.object({
  ...
  status: z.enum(['all', 'pending', 'completed', 'cancelled', 'refunded', 'partially_refunded']).optional(),
  customer_id: z.string().optional(),
  processed_by: z.string().optional(),
  payment_type: z.enum(['cash', 'card', 'digital_wallet', 'bank_transfer', 'store_credit', 'gift_card']).optional(),
  date_from: z.string().datetime().optional(),
  date_to: z.string().datetime().optional(),
  amount_min: z.number().min(0).optional(),
  amount_max: z.number().min(0).optional(),
  has_refund: z.boolean().optional(),
  sort_by: z.enum(['transaction_number', 'processed_at', 'total_amount', 'customer_name']).optional(),
