...
export function createSupplierStore() {
	...
	function toggleSupplierActive(id: string) {
		suppliers = suppliers.map((s) =>
			s.id === id
				? { ...s, is_active: !s.is_active, updated_at: new SvelteDate().toISOString() }
				: s
		);
	}

	function addPurchaseOrder(
		poData: Omit<PurchaseOrder, 'id'> & { items: Omit<PurchaseOrderItem, 'id'>[] }
	) {
