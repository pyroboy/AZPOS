...
function createPurchaseOrderStore() {
	...
	function updateReceivedQuantity(poId: string, itemId: string, received: number) {
		...
		if (poIndex !== -1) {
			const itemIndex = purchaseOrders[poIndex].items.findIndex(item => item.productId === itemId);
			if (itemIndex !== -1) {
	...
	return {
		...
		get inTransitOrders() { return inTransitOrders; },
		get arrivedOrders() { return arrivedOrders; },
		get partialOrders() { return partialOrders; },
		get completedOrders() { return completedOrders; },
