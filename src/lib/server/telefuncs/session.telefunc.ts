...
export async function onCreateSession(sessionData: unknown): Promise<SessionState> {
  try {
    ...
    const expiresAt = new Date(Date.now() + (validatedData.expires_in_minutes * 60 * 1000)).toISOString();
...
export async function onUpdateSession(sessionId: string, sessionData: unknown): Promise<SessionState> {
  try {
    ...
    if (validatedData.extend_expiry && validatedData.new_expiry_minutes) {
      updateData.expires_at = new Date(Date.now() + (validatedData.new_expiry_minutes * 60 * 1000)).toISOString();
    }
  ...
  try {
    ...
    Object.keys(stats.device_breakdown).forEach(device => {
      stats.device_breakdown[device].percentage = 
        stats.total_sessions > 0 ? (stats.device_breakdown[device].count / stats.total_sessions) * 100 : 0;
    });
