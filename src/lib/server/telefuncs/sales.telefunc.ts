...
export async function onGetSalesReport(filters?: SalesReportFilters): Promise<PaginatedSalesReport> {
  ...
  adjustmentsQuery = adjustmentsQuery.order(sortMapping[sortBy] || 'created_at', { ascending: sortOrder === 'asc' });
  ...
  const salesItems: SalesItem[] = (adjustments || [])
    .map(adjustment => transformAdjustmentToSaleItem(adjustment, productMap.get(adjustment.product_id)))
  ...
  if (validatedFilters.search && validatedFilters.product_name) {
    filteredSalesItems = salesItems.filter(item => 
      item.product_name.toLowerCase().includes(validatedFilters.search!.toLowerCase()) ||
      (item.product_sku && item.product_sku.toLowerCase().includes(validatedFilters.search!.toLowerCase()))
    );
  }
...
export async function onGetSalesStats(dateFrom?: string, dateTo?: string): Promise<SalesStats> {
  ...
  const salesData = (adjustments || [])
    .map(adjustment => ({
      ...adjustment,
      quantity: Math.abs(adjustment.quantity_adjusted),
      total_amount: adjustment.products ? Math.abs(adjustment.quantity_adjusted) * adjustment.products.price : 0
    }))
  ...
  // Calculate averages
  stats.average_transaction_value = stats.total_transactions > 0 ? stats.total_revenue / stats.total_transactions : 0;
  stats.average_items_per_transaction = stats.total_transactions > 0 ? stats.total_items_sold / stats.total_transactions : 0;
  ...
  const topSellingProducts = Array.from(productStats.values())
    ...
    .map(product => ({
      ...product,
      percentage_of_total: stats.total_revenue > 0 ? (product.revenue / stats.total_revenue) * 100 : 0
    }));
...
export async function onGetSalesSummary(period: 'today' | 'week' | 'month' | 'year' | 'custom' = 'today', customFrom?: string, customTo?: string): Promise<SalesSummary> {
  const { user } = getContext();
  if (!user) throw new Error('Not authenticated');
...
export async function onGetProductPerformance(dateFrom?: string, dateTo?: string, limit: number = 50): Promise<ProductPerformance[]> {
  const { user } = getContext();
  if (!user) throw new Error('Not authenticated');
  ...
  const totalQuantity = (adjustments || []).reduce((sum, adj) => sum + Math.abs(adj.quantity_adjusted), 0);
