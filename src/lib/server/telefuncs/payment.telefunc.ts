...
export async function onProcessPayment(paymentData: unknown): Promise<PaymentResult> {
  ...
  try {
    ...
    paymentResult.error_message = error instanceof Error ? error.message : 'Payment processing failed';
...
export async function onGetPayments(filters?: any): Promise<PaginatedPayments> {
  ...
  if (validatedFilters.search) {
    query = query.or(`reference.ilike.%${validatedFilters.search}%,transaction_id.ilike.%${validatedFilters.search}%`);
  }
...
export async function onGetPaymentStats(dateFrom?: string, dateTo?: string): Promise<PaymentStats> {
  ...
  stats.avg_payment_amount = stats.successful_payments > 0 ? stats.total_amount / stats.successful_payments : 0;
  ...
  Object.keys(stats.payment_method_breakdown).forEach(method => {
    ...
    methodStats.percentage = stats.total_amount > 0 ? (methodStats.total_amount / stats.total_amount) * 100 : 0;
    methodStats.avg_amount = methodStats.count > 0 ? methodStats.total_amount / methodStats.count : 0;
  });
