...
export async function onGetSuppliers(filters?: SupplierFilters): Promise<PaginatedSuppliers> {
  ...
  if (validatedFilters.search) {
    query = query.or(`name.ilike.%${validatedFilters.search}%,code.ilike.%${validatedFilters.search}%,email.ilike.%${validatedFilters.search}%`);
  }
...
export async function onUpdateSupplier(supplierId: string, supplierData: unknown): Promise<Supplier> {
  const { user } = getContext();
  if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
    throw new Error('Not authorized - admin/manager access required');
  }
...
export async function onGetSupplierStats(): Promise<SupplierStats> {
  ...
  const poStats = purchaseOrders?.reduce((acc, po) => {
  ...
  const topSuppliers = topSuppliersData?.map(supplier => ({
    id: supplier.id,
    name: supplier.name,
  ...
  return {
    ...
    ...poStats,
    avg_order_value: poStats.total_purchase_orders > 0 ? poStats.total_purchase_value / poStats.total_purchase_orders : 0,
    top_suppliers: topSuppliers
  };
}

// Telefunc to get supplier performance metrics
export async function onGetSupplierPerformance(supplierId: string, period: 'month' | 'quarter' | 'year'): Promise<SupplierPerformance> {
  const { user } = getContext();
  if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
    throw new Error('Not authorized - admin/manager access required');
  }
  ...
  const averageDeliveryTime = totalDeliveries > 0 ? performance.total_delay_days / totalDeliveries : 0;
