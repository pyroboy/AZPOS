...
export async function onCreateStockTransaction(transactionData: unknown): Promise<StockTransaction> {
  try {
    ...
    const stockTransaction: StockTransaction = {
      ...
      total_cost: validatedData.unit_cost ? Math.abs(validatedData.quantity_change) * validatedData.unit_cost : undefined,
  ...
  try {
    ...
    for (const adjustment of validatedData.adjustments) {
      ...
      const transaction: StockTransaction = {
        ...
        total_cost: adjustment.unit_cost ? Math.abs(adjustment.quantity_change) * adjustment.unit_cost : undefined,
        reference_type: validatedData.reference_type,
...
export async function onGetStockTransactionStats(): Promise<StockTransactionStats> {
  try {
    ...
    const baseStats = {
      ...
      movement_type_breakdown: {} as Record<string, { count: number; percentage: number; total_quantity: number }>,
      location_breakdown: {} as Record<string, { count: number; total_quantity_in: number; total_quantity_out: number }>,
      top_products: [] as Array<{ product_id: string; product_name: string; transaction_count: number; total_quantity_moved: number }>,
      cost_impact: {
        total_cost_adjustments: 0,
        positive_adjustments: 0,
        negative_adjustments: 0
      },
      hourly_activity: [] as Array<{ hour: number; transaction_count: number; total_quantity: number }>
    };
    ...
    const stats = transactions?.reduce((acc, transaction) => {
      ...
      acc.movement_type_breakdown[transaction.movement_type].total_quantity += Math.abs(transaction.quantity_change);
      ...
      if (transaction.location_id) {
        ...
        if (transaction.quantity_change > 0) {
          acc.location_breakdown[transaction.location_id].total_quantity_in += transaction.quantity_change;
        } else {
          acc.location_breakdown[transaction.location_id].total_quantity_out += Math.abs(transaction.quantity_change);
        }
      }
  ...
  try {
    ...
    transactions?.forEach(transaction => {
      ...
      if (!batchData.has(key)) {
        const ageInDays = Math.floor((now.getTime() - new Date(transaction.created_at).getTime()) / (1000 * 60 * 60 * 24));
        ...
        batchData.set(key, {
          ...
          batches: [{
            ...
            days_to_expiry: transaction.expiry_date ? 
              Math.floor((new Date(transaction.expiry_date).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)) : 
              undefined
          }]
        });
      }
    ...
    const agingReport: StockAging[] = Array.from(batchData.values()).map(data => {
      const totalQuantity = data.batches.reduce((sum: number, batch: any) => sum + batch.quantity, 0);
      const averageAge = data.batches.reduce((sum: number, batch: any) => sum + batch.age_days, 0) / data.batches.length;
