...
export async function onGetProductBatches(filters?: ProductBatchFilters): Promise<PaginatedProductBatches> {
  try {
...
export async function onUpdateProductBatch(batchId: string, batchData: unknown): Promise<ProductBatch> {
  try {
    const { user } = getContext();
    if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
      throw new Error('Not authorized - admin/manager access required');
...
export async function onAdjustBatchQuantity(batchId: string, quantityChange: number, reason?: string): Promise<ProductBatch> {
  try {
    const { user } = getContext();
    if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
      throw new Error('Not authorized - admin/manager access required');
...
export async function onGetProductBatchStats(): Promise<ProductBatchStats> {
  try {
    ...
    const totalCostValue = batches?.reduce((sum, batch) => 
      sum + (batch.purchase_cost || 0), 0) || 0;
