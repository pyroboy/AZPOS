...
<div class="p-4 sm:p-6">
	...
	<div class="bg-base-100 p-4 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold">Fast-Moving Items (Sold within last {fastMovingThreshold} days)</h2>
            <ExportButton 
				...
				<tbody>
					...
					{#each data.fastMovers as item (item.product_id)}
                        <tr>
                            ...
                            <td class="text-right font-mono">{new Date(item.last_sale_date).toLocaleDateString()}</td>
                        </tr>
                    {/each}
				</tbody>
			</table>
        ...
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold">Slow-Moving Items (Not sold in last {slowMovingThreshold} days)</h2>
            <ExportButton 
				...
				<tbody>
					...
					{#each data.slowMovers as item (item.product_id)}
                        <tr>
                            ...
                            <td class="text-right font-mono">{item.last_sale_date ? new Date(item.last_sale_date).toLocaleDateString() : 'N/A'}</td>
                        </tr>
                    {/each}
				</tbody>
			</table>
