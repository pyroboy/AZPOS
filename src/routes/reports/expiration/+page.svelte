...
<div class="space-y-4">
    ...
    <p class="text-muted-foreground">This report shows all product batches expiring within the next 90 days.</p>
            ...
            <Table.Root>
                ...
                <Table.Body>
                    {#if data.nearExpiryProducts.length === 0}
                        <Table.Row>
                            <Table.Cell colspan={4} class="text-center">No products are nearing expiration.</Table.Cell>
                        </Table.Row>
                    {:else}
                        {#each data.nearExpiryProducts as item}
                            <Table.Row>
                                ...
                                <Table.Cell>{item.batch_number}</Table.Cell>
                                                                <Table.Cell>{item.expiration_date ? formatDate(item.expiration_date) : 'N/A'}</Table.Cell>
