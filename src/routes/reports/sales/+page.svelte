    ...
    {#if error}
        <Card.Root class="border-red-200 bg-red-50">
            <Card.Content class="pt-6">
                ...
                <Button.Root variant="outline" size="sm" onclick={handleRetry} class="mt-3" disabled={isRetrying}>
                    <RefreshCw class="h-4 w-4 mr-2" class:animate-spin={isRetrying} />
                    Try again
                </Button.Root>
            </Card.Content>
                            ...
                            {:else}
                                {#each salesData() as sale}
                                    <Table.Row>
                                        ...
                                        <Table.Cell class="font-medium">{sale.totalSale.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</Table.Cell>
                                    </Table.Row>
                                {/each}
                            {/if}
                        </Table.Body>
