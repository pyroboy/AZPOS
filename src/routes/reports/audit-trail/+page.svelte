...
<div class="space-y-4">
    ...
    <p class="text-muted-foreground">A complete history of all stock movements, sorted by most recent.</p>
                    ...
                    {:else}
                        {#each data.transactions as tx}
                            <Table.Row>
                                ...
                                <Table.Cell>{formatTransactionType(tx.transaction_type)}</Table.Cell>
                                <Table.Cell>{tx.userName}</Table.Cell>
                                <Table.Cell class="text-right font-mono {tx.qty_change < 0 ? 'text-destructive' : tx.qty_change > 0 ? 'text-green-600' : ''}">
