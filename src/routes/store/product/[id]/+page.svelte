...
<script lang="ts">
	...
	function toggleModifier(modifierId: string, modifierData: { name: string; price_adjustment: number }): void {
...
<svelte:head>
	{#if product}
		...
		<meta name="description" content={product.description || `${product.name} - Available at AZPOS Store`} />
	{:else}
								...
								<CardContent>
									<div class="space-y-2">
										{#each product.available_modifiers as modifier}
											<label class="flex items-center justify-between p-2 border rounded cursor-pointer hover:bg-muted">
							...
							{#if selectedModifiers.length > 0}
								{#each selectedModifiers as modifier}
									<div class="flex justify-between text-sm text-muted-foreground">
										...
										<span>{modifier.price_adjustment > 0 ? '+' : ''}{formatPrice(modifier.price_adjustment)}</span>
									</div>
								{/each}
							{/if}
