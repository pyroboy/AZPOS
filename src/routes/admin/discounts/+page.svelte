                ...
                <TableBody>
                    {#each discounts as discount (discount.id)}
                        <TableRow>
                            ...
                            <TableCell>{discount.type === 'percentage' ? `${discount.value}%` : `${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(discount.value)}`}</TableCell>
                            <TableCell>
                            ...
                            <TableCell class="text-right">
                                ...
                                <Button variant="ghost" size="icon" onclick={() => openEditDiscountDialog(discount)} title="Edit">
                                    <Pencil class="h-4 w-4" />
                                </Button>
                                <Button variant="ghost" size="icon" class="text-destructive" onclick={() => handleDelete(discount.id)} title="Delete">
...
<Dialog bind:open={showDiscountDialog}>
    <DialogContent>
        <DialogHeader>
            ...
            <DialogDescription>
                {editingDiscount ? 'Update the details for this discount.' : 'Create a new discount for your store.'}
            </DialogDescription>
        </DialogHeader>
